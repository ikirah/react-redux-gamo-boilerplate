...
export const getOption = (value = '', options = []) => {
  for (const key in options) {
    if (options[key] && options[key].value && options[key].value === value) {
      return options[key]
    }
  }
  return ''
  ...
  if (!isEmpty(format)) {
    ...
    if (isNaN(Number(valueToNumber))) {
      ...
      try {
        ...
        const comma =
          decimalFormat.length > 0 && value.indexOf('.') >= 0 ? '.' : ''
        const decimal =
  ...
  if (value !== '' || rules.required) {
    for (const key in rules) {
      ...
      if (typeofRules === 'number' || typeofRules === 'string') {
        switch (key) {
          case 'required': {
            if (isEmpty(value)) return rules[key]
            break
          }
          case 'number': {
...
export const isValidForm = firstError => {
  return firstError === '' ? '' : 'disable'
}
